...
function MovieInfo({ watchList, handleAddToWatchList, handleRemoveFromWatchList }) {
    ...
    return (
        <>
            <div onClick={() => navigate(-1)} className="ml-20 mt-10 mx-10 w-[60px] h-[60px] rounded-4xl text-2xl bg-gray-100 flex justify-center items-center hover:cursor-pointer hover:scale-115 duration-200"><i className="fa-solid fa-arrow-left"></i></div>
            <div className="ml-60 mr-60 flex flex-row flex-wrap justify-center items-start">
                ...
                <div className="w-[1000px] flex flex-col flex-wrap items-start gap-3">
                    <h1 className="text-4xl font-bold text-center mt-5 justify-items-start">{movieData.title}</h1>
                    <p className="-ml-1 text-2xl"><i class="fa-solid fa-star fa-lg" style={{ color: '#FFD43B' }}></i> {Math.round(movieData.vote_average * 10) / 10}/10 - Votes:{movieData.vote_count}</p>
                    <ul className="flex flex-row flex-wrap justify-center items-start list-disc ml-7 gap-16 text-2xl">
